...
t.text(20, 54, "Das Flavin nimmt vom α-Kohlenstoff des Amins ein Hydrid auf; das entstehende "
               "Iminium zerfällt von selbst zum Aldehyd. Anders als NAD&#8314; kann Flavin "
               "denselben", size=12.5)
t.text(20, 73, "Vorgang auch in zwei Ein-Elektronen-Schritten führen — und genau das nutzen die "
               "irreversiblen Hemmstoffe aus.", size=12.5)

fox = mech.Molekuel(FLAVIN_OX, 190, 208, labels={14: "Ribityl", 20: "Cys"},
                    zeige={4: "rechts"}, name="FAD oxidiert")
t.mole.append(fox)
t.atomnummer(fox, 4, "N5", winkel=210, abstand=40, size=10.5, farbe=R, gewicht=700)
...
lp_am = t.elektronenpaar(amin, 2, 20)
t.pfeil(lp_am, amin.aussen(1, 2, abstand=22), bogen=0.40, seite=-1, farbe=W)
...
t.mole.append(fred)
t.atomnummer(fred, 14, "N5", winkel=290, abstand=40, size=10.5, farbe=R, gewicht=700)
t.unterschrift(fred, "FADH&#8322; — das N5 trägt jetzt den Wasserstoff", abstand=32)
...
t.text(227, 436, "&#8722; 2 H", size=10, anchor="middle", farbe=G)
...
sel = mech.Molekuel("C#CCN(C)C(C)Cc1ccccc1", 178, 730, zeige={0: "oben"}, name="Selegilin")
t.mole.append(sel)
...
t.unterschrift(sel, "Selegilin — die Propargylgruppe oben",
...
t.text(356, 722, "dann &#8722; H&#8226;", size=10, anchor="middle", farbe=R, gewicht=700)
...
t.mole.append(add)
t.atomnummer(add, 14, "N5", winkel=190, abstand=40, size=10.5, farbe=R, gewicht=700)
...
t.text(528, 1018, "Dieselbe Propargylgruppe, derselbe Mechanismus — aber", size=12.5)
t.text(528, 1037, "sie sitzt an einem Indangerüst statt an einem Amphetamin.", size=12.5)
...
ARIA = (
    ...
    "Alpha-Kohlenstoff des Amins auf; gleichzeitig bildet das Stickstoff-Elektronenpaar die "
    "Iminium-Doppelbindung. Es entsteht reduziertes Flavin, dessen N5 nun den Wasserstoff "
    "traegt. Auf der Substratseite zerfaellt das Iminium mit Wasser zum Aldehyd. Die "
    "Reoxidation des Flavins durch Sauerstoff liefert Wasserstoffperoxid. Zone B zeigt die "
    "Suizidinhibition: Vom Stickstoff des Selegilins geht mit einem Fischhakenpfeil ein "
    "einzelnes Elektron auf das Flavin ueber; nach Verlust eines Wasserstoffatoms addiert der "
    "Rest kovalent an das N5, und es entsteht das gezeichnete N5-Addukt, das den Cofaktor "
    "endgueltig blockiert. Zone C zieht die therapeutischen Folgerungen."
)
...
daten["m06"] = t.svg(ARIA)
io.open(ZIEL, "w", encoding="utf-8").write(
    json.dumps(daten, ensure_ascii=False, indent=1))
print("geschrieben: tafeln.json / m06  (%d Zeichen, %d Molekuele, %d Pfeile)"
      % (len(daten["m06"]), len(t.mole), len(t.anker)))
